          ...
          if (PATH.basename(file) !== "notes.html") {
            ...
            if (COMPILE_NOTES === true) {
              ...
              for (i=1; i<=Object.keys(JSON.parse(DATA)).length; i++) {
                ...
                // Set - Source (Image)
                if (PATH.extname(JSON.parse(DATA)[i].image) === ".jpg") {
                  var content3 = content2.replace('id="video" class="', 'id="video" class="d-none ');
                  var content4 = content3.replace('src="/img/notes-placeholder.png" alt="Cover Image"', 'src="/img/' + JSON.parse(DATA)[i].image + '" alt="Cover Image"');
                } else if (PATH.extname(JSON.parse(DATA)[i].image) === ".mp4") {
                  var content3 = content2.replace('id="image" class="', 'id="image" class="d-none ');
                  var content4 = content3.replace('src="/img/notes-placeholder.mp4" type="video/mp4"', 'src="/img/' + JSON.parse(DATA)[i].image + '" type="video/mp4"');
                }
